...
Use the packet's requested `artifact_target`. Default to a Markdown file attached/available in the Worker chat. If the target is optional Google Drive and Drive is unavailable, fall back to a chat file without blocking the research. If file creation is unavailable entirely, render the Artifact inline in the Worker chat, label the fallback clearly, and set `artifact_ref` to that inline Artifact location.
