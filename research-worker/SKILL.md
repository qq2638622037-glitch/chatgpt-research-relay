...
5. Create the full Evidence Artifact using `references/artifact-contract.md`. Preserve the deterministic `<task_id>_evidence.md` filename so a later Master audit can resolve the file through ChatGPT Library when direct chat links do not cross chats or projects.
...
- Never fabricate a persistent Library, Drive, project, or connector identifier. If the host does not expose a durable reference, use the deterministic Artifact filename as `artifact_ref` rather than relying on a bare sandbox path as the only locator.
