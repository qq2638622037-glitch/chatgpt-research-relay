...
5. Read the full Evidence Artifact only when the intake rules require it or the user asks for an audit. When a direct Worker-chat reference is unavailable, resolve the deterministic Artifact filename through ChatGPT Library before asking the user to transfer a file manually.
...
- Do not make Google Drive or another connector a hidden requirement. Use ChatGPT Library resolution when available and keep manual handoff as the final fallback.
