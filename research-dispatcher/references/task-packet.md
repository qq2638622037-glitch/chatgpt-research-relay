...
Default `artifact_target` to `chat_file`. In ChatGPT, the deterministic Artifact filename also serves as the cross-chat Library lookup key when a direct Worker-chat reference is unavailable. Change the target only when the user explicitly requests another supported destination.
